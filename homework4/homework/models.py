...
class Detector(torch.nn.Module):
    ...
    def detect(self, image):
        ...
        smaller_img = self_image.squeeze(0)
        for i in range(len(smaller_img)):
          cur_peak_list = extract_peak(smaller_img[i], max_det=30)
          class_list = []
          for cur_peak in cur_peak_list:
            peak_info = []
            peak_info.append(cur_peak[0])
            peak_info.append(cur_peak[1])
            peak_info.append(cur_peak[2])
            peak_info.append(0)
            peak_info.append(0)
            class_list.append(tuple(peak_info))
          detect_list.append(class_list)
        return detect_list
