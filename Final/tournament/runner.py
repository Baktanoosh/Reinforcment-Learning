...
def to_image(x, proj, view):
    '''
    x: 3-dim world coordinate
    normalize: True (normalize)
    '''
    raw = proj @ view @ np.array(list(x)+[1]) # raw screen coordinates
    norm = raw/raw[-1] # normalized screen coordinates
    #screen = np.clip(np.array([norm[0], -norm[1]]), -1, 1)
    screen = np.array([norm[0], -norm[1]])
    return raw,norm,screen         

def to_world(raw, proj, view):
    world = np.linalg.inv(view) @ np.linalg.inv(proj) @ raw
    return world
...
class Match:
    ...
    """
    def __init__(self, use_graphics=True, logging_level=None, training=False):
        '''
        training: True (generating images and labels during gameplay for training), False (does not generate)
        '''
        ...
        if use_graphics:
            ...
            # graphics_config.screen_width = 128*2
            # graphics_config.screen_height = 96*2
    ...
    def run(self, team1, team2, num_player=1, max_frames=MAX_FRAMES, max_score=3, record_fn=None, timeout=1e10,
        ...
        for it in range(max_frames):
            ...
            if self._use_graphics:
                ...
                if self.training:
                    for i,img in enumerate(team1_images+team2_images):         
                        if i%2 == 0:
                            idx = i//2
                        else: 
                            idx = i//2 + 2 
                        proj = np.array(state.players[idx].camera.projection).T
                        view = np.array(state.players[idx].camera.view).T
                        puck_coord = soccer_state['ball']['location']
                        raw, norm, puck_center = to_image(puck_coord, proj, view)
                        # print('puck_coord: ', puck_coord)
                        # print('raw: ', raw)
                        # print('puck_recov: ', to_world(raw, proj, view))
                        # w = to_world(norm, proj, view)
                        # print('puck_recv2: ', w/w[-1])
                        # exit()

                        if any(abs(i) > 1.0 for i in norm): # decide if the puck exists in the screen or not
                            puck_exist = 0 
                        else: 
                            puck_exist = 1
                        # saving files
                        
                        with open(f'{self.directory}/{it}_{i}.csv', 'w') as f:
                            f.write('%0.2f,%0.2f,%0.2f,%0.2f,%0.2f,%0.2f,%0.1f' % tuple(np.append(np.append(norm, puck_center),puck_exist)))
                        Image.fromarray(img).save(f'{self.directory}/{it}_{i}.png')
                        
                        # puck_screen, puck_center = 
                        # puck_center = utils.PyTux._to_image(puck_coord, proj, view)
                        # if any(abs(coord) == 1.0 for coord in puck_center):
                        #     puck_exist = 0
                        # else:
                        #     puck_exist = 1
                        # with open(f'{self.directory}/{it}_{i}.csv', 'w') as f:
                        #     f.write('%0.2f,%0.2f,%0.1f' % tuple(np.append(puck_center, puck_exist)))
